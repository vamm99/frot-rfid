...
export const useClientStore = defineStore("client", {
  state: () => ({
    ...
    saldoAdicional: "",
    clientForm: {
      // Objeto que contiene todos los datos del formulario
      nombre: "",
      apellido: "",
      cc: "",
      telefono: "",
  ...
  actions: {
    ...
    async fetchClients(params = {}) {
      try {
        // Solicitud GET usando Axios con baseURL configurada
        const response = await axios.get("/fetchUser", { params });
    ...
    async recargarSaldo() {
      try {
        ...
        if (error.response && error.response.data) {
          this.messageApiRecargarSaldo =
            "Error al recargar puntos: " + error.response.data.message;
        } else {
          this.messageApiRecargarSaldo =
            "Error al recargar puntos: " + error.message;
        }
        throw error;
      }
    },
    ...
    async descontarPuntos(puntos) {
      try {
        // Hacer la solicitud PUT a la API para actualizar el saldo
        const response = await axios.put(
          `/${this.rfidReadRecargarSaldo}/saldo`,
          {
            saldo_adicional: puntos,
          }
        );
    ...
    async registerClient() {
      ...
      try {
        ...
        if (error.response && error.response.data) {
          this.messageApi =
            "Error al registrar cliente: " + error.response.data.message;
        } else {
    ...
    async updateClient() {
      ...
      try {
        // Hacer la solicitud POST a la API para registrar al cliente
        const response = await axios.put(
          `/update/${this.clientFormUpdate.cc}`,
          clientFormUpdate
        );
    ...
    async searchClient(query) {
      try {
        ...
        if (error.response && error.response.data) {
          this.messageApiUsuarioEncontrado =
