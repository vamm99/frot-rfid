  ...
  <div class="container mt-5 p-5">
    <div class="row p-5 my-5">
      <div class="col-12 col-md-5 my-5">
        ...
        <div
          v-if="client.message && !client.messageApiRecargarSaldo"
          :class="[
            'alert',
            `alert-${client.clase}`,
            'alert-dismissible',
        ...
        <div
          v-if="client.messageApiRecargarSaldo"
          :class="[
            'alert',
            'alert-success',
            'alert-dismissible',
        ...
        >
          ...
          <button
            type="button"
            class="btn-close"
            @click="client.messageApiRecargarSaldo = null"
            aria-label="Close"
          ></button>
        </div>
        <form @submit.prevent="submitForm" class="needs-validation" novalidate>
          <div class="mb-3">
            ...
            <input
              ...
              class="form-control"
              id="uid"
              v-model="client.rfidReadRecargarSaldo"
              readonly
            />
          </div>
        ...
        <button @click="descontar" class="btn btn-warning w-100">
          Descontar corte gratis
        </button>
      </div>
      <div class="col-12 col-md-7 my-5">
        ...
        <div class="neumorphic-card mt-4">
          <div class="card-content">
            ...
            <p>Puntos: {{ client.saldo_acumulado }}</p>
            <p
              v-if="Number(p) <= client.saldo_acumulado"
              style="color: green; font-weight: bold"
            >
              Es hora de reclamar su corte gratis
            </p>
          </div>
          <div>
            <img class="img" src="/antena.png" alt="" />
          </div>
        </div>
      </div>
    </div>
...
const { puntosGet } = useAuthStore();
const p = ref(0);

const m = async () => {
  const puntos = await puntosGet();
  console.log(puntos);
  p.value = puntos;
};
...
const descontar = async () => {
  ...
  if (puntos) {
    try {
      await client.descontarPuntos(-puntos);
      // Llamamos a la acción que recarga el saldo
    } catch (error) {
  ...
  echo.channel("rfid-tag-read").listen("RFIDTagReadEvent", (e) => {
    if (e.userData) {
      client.updateUID(
        ...
        e.userData.telefono,
        e.userData.email,
        e.userData.saldo_acumulado,
        e.message,
        "success"
      ); // Actualizar el UID en el store
    } else {
